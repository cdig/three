...
// MAIN ############################################################################################
var AJAX, ajaxSuccess, canvas, colorBuffer, colorLocation, createProgram, createShader, degToRad, drawScene, gl, m4, main, matrixLocation, positionBuffer, positionLocation, random, resize, rotation, scale, setColors, setGeometry, translation;
...
setColors = function() {
  ...
  return gl.bufferData(gl.ARRAY_BUFFER, colors, gl.STATIC_DRAW);
};
...
main = function(program) {
  ...
  setColors();
  return drawScene(program);
};

drawScene = function(program) {
  var bottom, far, left, matrix, near, normalize, offset, right, size, stride, top, type;
  resize();
  ...
  // RENDER
  return gl.drawArrays(gl.TRIANGLES, 0, 16 * 6);
};
...
m4 = {
  ...
  multiply: function(a, b) {
    var a00, a01, a02, a03, a10, a11, a12, a13, a20, a21, a22, a23, a30, a31, a32, a33, b00, b01, b02, b03, b10, b11, b12, b13, b20, b21, b22, b23, b30, b31, b32, b33, dst;
    b00 = b[0 * 4 + 0];
    b01 = b[0 * 4 + 1];
    b02 = b[0 * 4 + 2];
    b03 = b[0 * 4 + 3];
...
ajaxSuccess = function(xhr, opts) {
  return function(e) {
    if (xhr.status === 200) {
      return opts.success(xhr.response, e, xhr);
    } else {
      return console.log("AJAX Failure");
    }
  };
};
...
createShader = function(source, type) {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    throw `failed compile: ${gl.getShaderInfoLog(shader)}`;
  }
  return shader;
};
...
createProgram = function(vertexSource, fragmentSource) {
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    throw `failed link: ${gl.getProgramInfoLog(program)}`;
  }
  return program;
};
...
AJAX({
  url: "main.fshader",
  success: function(fragmentSource) {
    return AJAX({
      url: "main.vshader",
      success: function(vertexSource) {
        return main(createProgram(vertexSource, fragmentSource));
      }
    });
  }
});
